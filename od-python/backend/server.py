...
async def update_settings(payload: dict = Body(...)):
    ...
    if "accdb_path" in allowed:
        ...
        # Pre-download JDBC jars in the background so the fast SQL read path is
        # ready before the user presses "Set as target" (needs Java installed).
        try:
            import asyncio
            from engine import accdb_bridge
            if accdb_bridge.java_available():
                asyncio.create_task(
                    asyncio.to_thread(accdb_bridge.ensure_ucanaccess))
        except Exception:
            pass
...
async def _resolve_target_rows(source, ident):
    ...
    if source == "tool":
        ...
        colnum_to_header = await _entete_colnum_map(cfg)
        raw = await asyncio.to_thread(
            accdb_bridge.read_project_events, path, rec.get("ID"),
...
async def targets_apply_multi(payload: dict = Body(...)):
    ...
    async def _apply_one(t):
        try:
            vehicle, rows = await _resolve_target_rows(t.get("source"), t.get("id"))
            return {"ok": True, "vehicle": vehicle, "rows": rows}
        except HTTPException as e:
            return {"ok": False, "target": t.get("label"), "detail": e.detail}

    import asyncio
    sem = asyncio.Semaphore(3)
    async def _bounded(t):
        async with sem:
            return await _apply_one(t)
    results = await asyncio.gather(*[_bounded(t) for t in targets])
    for r in results:
        if not r.get("ok"):
            errors.append({"target": r.get("target"), "detail": r.get("detail")})
            continue
        vehicle, rows = r["vehicle"], r["rows"]
        labels.append(vehicle)
        multi[vehicle] = {row["sdv"].strip().upper(): row for row in rows}
    if not multi:
...
async def targets_set_pending(payload: dict = Body(...)):
    ...
    if pend:
        ...
        # Pre-warm: score pending targets in the background so "Set as target"
        # is instant when the user reaches the RATING tab.
        import asyncio
        asyncio.create_task(_prewarm_pending_targets(list(pend)))
    return {"ok": True, "pending": len(pend),
            "labels": [p.get("label") for p in pend]}


async def _prewarm_pending_targets(pend):
    """Background: score pending comparison targets into the disk cache."""
    import asyncio
    sem = asyncio.Semaphore(3)
        ...
        async with sem:
            try:
                if t.get("source") == "ref":
                    ...
                    ident = t.get("id")
                await _resolve_target_rows(t.get("source"), ident)
            except Exception:
                pass

    await asyncio.gather(*[_one(t) for t in pend])
